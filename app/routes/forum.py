...
def surveyEdit(surveyID):
    editBlog = Survey.objects.get(id=surveyID)
    ...
    if current_user != editSurvey.author:
        flash("You can't edit a survey response you don't own.")
        return redirect(url_for('survey',surveyID=surveyID))
    # get the form object
    form = SurveyForm()
    ...
    form.subject.data = editSurvey.subject
    form.content.data = editSurvey.content
    form.tag.data = editSurvey.tag
...
# @app.route('/comment/new/<surveyID>', methods=['GET', 'POST'])
# # @login_required
